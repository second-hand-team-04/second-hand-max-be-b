...
domains=(fishprince.site www.fishprince.site)
rsa_key_size=4096
data_path="./certbot"
email="rjswmtk@gmail.com" # Adding a valid address is strongly recommended
...
docker-compose -f docker-compose-deploy.yml run --rm --entrypoint "\
  certbot certonly --webroot-path /usr/share/nginx/html \
    $staging_arg \
    $email_arg \
    $domain_args \
